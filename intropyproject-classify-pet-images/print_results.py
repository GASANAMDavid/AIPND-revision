...
def print_results(results_dic, results_stats_dic, model,print_incorrect_dogs=False, print_incorrect_breed=False):
    ...
    """
    print('Result Summary for CNN Model Architecture:  {}\n'.format(model.upper()))
    print("*" * 45, "\n")
    ...
    print("\nSummary Statistics(PERCENTAGES) on {} run ".format(model))
    print("*" * 35, "\n")

    # iterate through results_stats_dic printing percentages
    for key, val in results_stats_dic.items():
        if key.startswith('p'):
            print("{}: {}".format(key, val))

    # if print_incorrect is true identify incorrectly classfied Dogs
    if (print_incorrect_dogs and (
        ...
        for key, val in results_dic.items():
            if sum(val[3:]) == 1:
                count += 1
                print("{}. Pet Label: {} \t Classfier Label: {}".format(count, val[0], val[1]))
    ...
    if print_incorrect_breed and (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']):
        ...
        for key, val in results_dic.items():
            ...
            if sum(results_dic[key][3:]) == 2 and results_dic[key][2] == 0:
                count += 1
                print("{}. Real: {}   Classifier: {}".format(count, results_dic[key][0], results_dic[key][1]))
    None
